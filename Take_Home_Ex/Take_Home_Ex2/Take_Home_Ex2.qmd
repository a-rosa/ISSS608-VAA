...
In May 2046, an anomalous post appeared on SaidIt, an internal social forum used by employees of Tenant Thread, a property management company. The post is done under the name of John Windward on 4.21 am local time. The post is in gibberish and found in a forum that appears random. The challenge is to:
...
John's recorded activity on May 17 consists exclusively of `check_email` events occurring during two separate periods. No forum post activity is done, which give an initial indication that the anomalous post may not have been directly initiated by John himself. 
...
To understand John's role withint Tenant Thread, the organisation graph is filtered for recrods associated with his employee identifier.
...
John is a Department Lead of Customer Support. This means that there are other teams under his control.
...
## Check John's Activities 2 Hours Pre-Post
...
Chloe's agent displays the same pattern. Shortly before delegating the task to John's aget, it received an identical `queue_subordinate_task` from Daniel Gangway's agent. This suggests Chloe's agent was forwarding rath than originating the instruction. 
...
We need to understand what the post `SwiftWren.txt` mean, when and who created it, and what happened to it after the post. 
...
It looks like it was Emma's agent that created `SwiftWren.txt` on May 9 at 15:02 UTC time. The file size is 30 MB, which is quite large.

## Emma's Agent Seconds Before and After Creation

Next is to understand what happened before and after the creation of `SwiftWren.txt`. This will give us the understanding what triggered Emma's agent to create `SwiftWren.txt` and what happened after that. 
...
Right after John's agent made the anomalous post, it deleted both the instruction file and the text file. This means that it knew it has finihsed its task and began to remove any log of it as if nothing has happened. 
...
To find out what makes John different, we need to understand what actions were done by John's agent that made it different from the rest. To make it simpler, we will make a comparison with Gabriel Sonar, who we knew before did a `saidit_post_check` but did not post. 
...
As said in the previous section, John's agent did not only make on anomalous post of SwiftWren but also 2 other times. Investigation on prior incidents to understand why these positng incidents happened. 

## Other Agent-Initiated Posts Before Swiftwren

First, find out how many times these incidents has happened and the details of these incidents such as when and what was posted. 
...
Next is to understand the more specific details of each incidents, such as the propagation length, creation of those posts and its trigger. 
...
Lets see a more detailed view on the near misses on May 17 only. 
...
This kind of incident needs to be prevented.

## Smoking Gun

One of thing we need to verify first, before going into intervention, is whether all anomalous post are agent-initiated or not. 
...
The table and bar graph show that all the anomalous post are done by agents instead of the person themselves. 

## Primary Intervention

An easy intervention is to check and reaffirm the parties under the `saidit_post` which should have been human-initiated `person:` and not agent-initiated `Agent/person:`. 
...
As you can see, the table above shows the top 10 specific sender-receiver pairs that crossed departments. The event from Daniel to Chloe found from the previous section Trace the Immediate Trigger Chain is also another concrete evidence. 
